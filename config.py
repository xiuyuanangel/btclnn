...
# ==================== 类别不平衡处理 ====================
# 实测三类分布(data/cache 全量, LABEL_MIN_RETURN=0.001, 平滑窗口3, BTC+ETH合并):
#   [10min] 涨 26.98% / 平 46.60% / 跌 26.42%
#   [30min] 涨 34.15% / 平 32.68% / 跌 33.17%
#   [60min] 涨 38.19% / 平 24.94% / 跌 36.88%
# 结论: 「平」类占比 24.9%~46.6%, 完全不属于少数类(远高于15%经验线),
#       因此无需重采样。模型此前从不预测「平」的真正原因是损失里
#       对中性类额外乘了 0.5 的硬编码降权(见 NEUTRAL_WEIGHT_SCALE)。
USE_CLASS_WEIGHTS = True         # 是否在损失中启用类权重(按训练集标签分布计算); False=全部权重为1
CLASS_WEIGHT_SCHEME = 'inv_count'  # 类权重方案: 'inv_count' = total/(C*count) (各类梯度贡献均等)
                                   #             'inv_sqrt'  = 1/sqrt(count) 归一化(更温和)
NEUTRAL_WEIGHT_SCALE = 1.0       # 中性类(平)权重的额外缩放系数。
                                 # 历史值为 0.5, 它在「平」本就占比最高的情况下再砍半,
                                 # 直接导致中性Recall=0.0000(模型从不输出平)。默认改为 1.0 不额外干预。
USE_CLASS_BALANCED_SAMPLER = False  # 是否启用 WeightedRandomSampler 按类反比重采样。
                                    # 实测分布并不极端(平≥24.9%), 默认关闭; 仅当某类占比<5%时才建议开启。

# ==================== 预测信号口径 ====================
# 统一预测信号阈值: predict.py 打印方向、notifier.py 推送文案、
# verify_predictions.py 验证三处共用同一口径, 避免「屏幕说涨/推送喊强烈看跌」的矛盾。
# 方向一律取 argmax(最大概率类), 强度一律取置信度(最大类概率)。
CONF_THRESH = 0.4            # 置信度下限: 最大类概率 < 该值时判定为「信号弱」, 文案标注中性观望
CONF_STRONG = 0.7            # 强信号阈值: 最大类概率 ≥ 该值时文案用「强烈看涨/看跌」
CONF_MODERATE = 0.55         # 中等信号阈值: 最大类概率 ≥ 该值时文案用「看涨/看跌」
